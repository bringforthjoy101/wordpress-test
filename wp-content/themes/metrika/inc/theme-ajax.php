...
function getFileTitle() {
    ...
    echo mb_substr($attachment, strrpos($attachment, '/') + 1);
    die();
}
...
function blogPosts() {
    ...
    foreach ($posts as $key => $value) {
        ...
        if ($thumbnail && strlen($content) > 150)
            $content = the_excerpt_max_charlength( $content, 150 );
        elseif (strlen($content) > 189)
            $content = the_excerpt_max_charlength( $content, 189 );
    ...
    foreach ($posts as $key => $value) {
        ...
        <article class="post pt-page pt-page-<?php echo $i; ?>" 
                    id="<?php echo $slug ?>"
                    originalclasslist="post pt-page pt-page-<?php echo $i++; ?>">
